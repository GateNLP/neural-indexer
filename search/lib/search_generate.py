...
def generate_saved_search(embedding):
    ...
    saved_search["attributes"]["kibanaSavedObjectMeta"]["searchSourceJSON"] = search_source_s
...
def save_search(search_obj):
    ...
    req = requests.post(f"{config.kibana_host}/api/saved_objects/search", auth=auth, json=search_obj, headers=headers)
    resp = req.json()
    id = resp['id']
    return id